...
public class SBI implements BankInterface{
    ...
    public String withdrawMoney(int drawMoney, String password) {
        if(password==this.password){
            if(balance>drawMoney){
                balance-=drawMoney;
                return "Money withdrawn successfully the remaining balance is "+ balance;
            }
            else return "Insufficient balance current balance is "+ balance;
        }
    ...
    public double calculateTotalInterest(int years) {
        double interest=this.balance*this.rateOfInterest*years/100;
        return interest;
    }
}
